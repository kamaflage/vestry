...
function deleteTransaction(id) {
  ...
  if (t) { moveToTrash('transaction', t, t.desc + ' \u2014 ' + fmt(t.amt)); toast('Deleted. Undo in Config \u2192 Trash.', 'ok'); }
  save(); render();
}
...
function deleteRecurring(id) {
  ...
  if (r) { moveToTrash('recurring', r, r.name); toast('Deleted. Undo in Config \u2192 Trash.', 'ok'); }
  save(); render();
}

// ── AUTO-GENERATE RECURRING BILLS (v2.49) ──────────────────────────────────
// Never silent: this only ever creates transactions when the user reviews
// a checklist and explicitly confirms. Generated transactions are tagged
// autoGenerated so they're distinguishable in the ledger, and each bill
// records lastGeneratedYM so it won't be offered again this month \u2014 but
// nothing stops a user from also just logging a bill manually as before;
// this is purely an optional shortcut, not a requirement.
function updateRecurringGenerateBanner() {
  var banner = document.getElementById('rec-generate-banner');
  var btn = document.getElementById('rec-generate-btn');
  if (!banner || !btn) return;
  var candidates = recurringGenerationCandidates();
  var dueCount = candidates.filter(function(c){ return c.due; }).length;
  if (candidates.length === 0) {
    banner.style.display = 'none';
    btn.textContent = '\ud83d\udcc5 Generate This Month\u2019s Bills';
  } else {
    banner.style.display = 'block';
    banner.textContent = dueCount > 0
      ? dueCount + ' bill' + (dueCount===1?'':'s') + ' due this month ' + (dueCount===1?'hasn\u2019t':'haven\u2019t') + ' been logged yet.'
      : 'All bills logged for now \u2014 none are due yet, but you can generate early if you want.';
    btn.textContent = '\ud83d\udcc5 Generate This Month\u2019s Bills (' + candidates.length + ')';
  }
}

function openGenerateRecurringModal() {
  var candidates = recurringGenerationCandidates();
  var list = document.getElementById('genrec-list');
  if (candidates.length === 0) {
    list.innerHTML = '<div style="text-align:center;padding:16px;color:var(--text3);font-size:11px">Every recurring bill has already been generated for this month.</div>';
  } else {
    list.innerHTML = candidates.map(function(c, i){
      var b = c.bill;
      var dueTag = c.due ? '<span style="color:var(--expense);font-size:9px">due</span>' : '<span style="color:var(--text3);font-size:9px">not due yet</span>';
      return '<label style="display:flex;align-items:center;gap:10px;padding:8px;border:1px solid var(--border);border-radius:var(--r2);cursor:pointer">' +
        '<input type="checkbox" class="genrec-cb" data-idx="' + i + '" ' + (c.due ? 'checked' : '') + ' style="width:16px;height:16px;margin:0">' +
        '<div style="flex:1"><div style="font-size:12px">' + esc(b.name) + ' ' + dueTag + '</div>' +
        '<div style="font-size:9px;color:var(--text3)">' + esc(b.cat) + ' \u00b7 due day ' + b.day + ' \u00b7 ' + fmt(b.amt) + '</div></div>' +
        '</label>';
    }).join('');
  }
  document.getElementById('modal-genrecurring').classList.add('open');
}

function generateRecurringSelected() {
  var candidates = recurringGenerationCandidates();
  var checkboxes = document.querySelectorAll('.genrec-cb');
  var created = 0;
  checkboxes.forEach(function(cb){
    ...
    STATE.transactions.push({
      id: Date.now() + Math.random(), date: recurringDueDateThisMonth(b), desc: b.name, cat: b.cat,
      type: 'expense', amt: b.amt, owner: '', source: 'checking', autoGenerated: true
    });
    b.lastGeneratedYM = c.ym;
    created++;
  });
  STATE.transactions.sort(function(a,b){ return b.date.localeCompare(a.date); });
  save();
  closeModal('genrecurring');
  render();
  toast(created ? ('Generated ' + created + ' transaction' + (created===1?'':'s') + '.') : 'Nothing selected.', created ? 'ok' : 'error');
}
...
function deleteGoal(id) {
  ...
  if (g) { moveToTrash('goal', g, g.name); toast('Deleted. Undo in Config \u2192 Trash.', 'ok'); }
  save(); render();
}
...
function deleteWorkHour(id) {
  ...
  if (w) { moveToTrash('workHour', w, w.date + ' \u2014 ' + w.hours + 'h'); toast('Deleted. Undo in Config \u2192 Trash.', 'ok'); }
...
function deleteMileage(id) {
  ...
  if (m) { moveToTrash('mileage', m, m.date + ' \u2014 ' + m.distance.toFixed(1) + ' ' + distUnit()); toast('Deleted. Undo in Config \u2192 Trash.', 'ok'); }
  save(); renderWorkHoursPage();
}
...
function deleteExpense(id) {
  ...
  if (x) moveToTrash('expense', x, x.date + ' \u2014 ' + x.store);
  save(); renderWorkHoursPage();
  toast('Expense deleted. Undo in Config \u2192 Trash.', 'ok');
}
...
function deleteInvoice(id) {
  ...
  if (inv.transactionId) {
    var linkedTxn = STATE.transactions.find(function(t){ return t.id === inv.transactionId; });
    STATE.transactions = STATE.transactions.filter(function(t){ return t.id !== inv.transactionId; });
    if (linkedTxn) moveToTrash('transaction', linkedTxn, linkedTxn.desc + ' \u2014 ' + fmt(linkedTxn.amt));
  }
  ...
  moveToTrash('invoice', inv, inv.number);
  save();
  renderWorkHoursPage();
  render();
  toast('Invoice deleted. Undo in Config \u2192 Trash.', 'ok');
}
...
function saveConfig() {
  ...
  // Mileage unit: changing this used to just relabel every past distance
  // (a logged "100 km" instantly became "100 mi" with no conversion). Offer
  // to actually convert existing entries instead \u2014 distance AND rate both,
  // inversely, so distance\u00d7rate (the dollar amount) is unchanged either way.
  var oldMileageUnit = STATE.config.mileageUnit || 'km';
  var newMileageUnit = document.getElementById('cfg-inv-mileage-unit').value || 'km';
  if (oldMileageUnit !== newMileageUnit && STATE.mileage.length > 0) {
    var muFactor = (oldMileageUnit === 'km' && newMileageUnit === 'mi') ? 0.621371
                  : (oldMileageUnit === 'mi' && newMileageUnit === 'km') ? 1.609344
                  : 1;
    if (muFactor !== 1) {
      var doConvertMileage = confirm(
        'You have ' + STATE.mileage.length + ' logged mileage entr' + (STATE.mileage.length===1?'y':'ies') + ' in ' + oldMileageUnit + '.\n\n' +
        'Convert them to ' + newMileageUnit + ' now, so the distances and $/' + newMileageUnit + ' rates stay accurate? Dollar totals won\u2019t change either way \u2014 this only affects how the numbers are displayed.\n\n' +
        'OK = convert existing entries. Cancel = just relabel going forward (old entries will show the wrong numbers under the new unit).'
      );
      if (doConvertMileage) {
        STATE.mileage.forEach(function(m) {
          m.distance = m.distance * muFactor;
          if (m.odoStart != null) m.odoStart = m.odoStart * muFactor;
          if (m.odoEnd != null) m.odoEnd = m.odoEnd * muFactor;
          if (m.rate) m.rate = m.rate / muFactor;
        });
        toast('Converted ' + STATE.mileage.length + ' mileage entries from ' + oldMileageUnit + ' to ' + newMileageUnit + '.', 'ok');
      }
    }
  }
  STATE.config.mileageUnit        = newMileageUnit;
...
function trashTypeLabel(type) {
  return { transaction:'Transaction', recurring:'Recurring Bill', goal:'Savings Goal', workHour:'Work Hours', mileage:'Mileage', expense:'Expense Log', invoice:'Invoice' }[type] || type;
}
function trashTimeAgo(ts) {
...
function renderTrashList() {
  ...
  if (!list) return;
  var items = STATE.trash || [];
  var countEl = document.getElementById('trash-count');
  var emptyBtn = document.getElementById('trash-empty-btn');
  if (countEl) countEl.textContent = items.length ? (items.length + ' item' + (items.length===1?'':'s') + ' \u00b7 auto-removed after ' + TRASH_RETENTION_DAYS + ' days') : 'Empty';
  if (emptyBtn) emptyBtn.style.display = items.length ? '' : 'none';
  list.innerHTML = items.length === 0
    ? '<div style="text-align:center;padding:16px;color:var(--text3);font-size:11px">Trash is empty.</div>'
    : items.map(function(t){
        return '<div class="budget-goal-row"><div><div style="font-size:11px">' + esc(trashTypeLabel(t.type)) + ' \u00b7 ' + esc(t.label||'') + '</div><div style="font-size:9px;color:var(--text3)">' + trashTimeAgo(t.deletedAt) + '</div></div>' +
          '<div style="display:flex;gap:6px;align-items:center"><button class="btn" style="padding:4px 10px;font-size:10px" onclick="restoreTrashItem(' + t.trashId + ')">\u21b6 Restore</button>' +
          '<button class="del-btn" onclick="deleteTrashItemForever(' + t.trashId + ')" title="Delete forever">\u2715</button></div></div>';
      }).join('');
}
function restoreTrashItem(trashId) {
  var ok = restoreFromTrash(trashId);
  if (ok) {
    updateCategoryDropdowns();
    render();
    renderWorkHoursPage();
...
function mergeSkipDedup() {
  var cb = document.getElementById('merge-skip-dedup');
  return !!(cb && cb.checked);
}
function refreshMergePreview(fileName) {
  if (!_mergeData) return;
  var data = _mergeData;
  if (fileName) data._fileName = fileName;
  var skipDedup = mergeSkipDedup();
  var incoming = data.transactions || [];
  var dupes = 0, newCount = 0;
  incoming.forEach(function(t){
    var isDupe = !skipDedup && STATE.transactions.some(function(e){ return e.date===t.date && e.desc===t.desc && parseFloat(e.amt)===parseFloat(t.amt); });
  ...
  document.getElementById('merge-preview-text').innerHTML =
    (data._fileName ? '<strong>File:</strong> '+esc(data._fileName)+'<br>' : '') +
    ...
    (skipDedup
      ? '<span style="color:var(--oneoff)">\u26a0 Dedup off \u2014 all '+incoming.length+' rows will import</span>'
...
Vestry.UI = {
  ...
  updateRecurringGenerateBanner: updateRecurringGenerateBanner,
  openGenerateRecurringModal: openGenerateRecurringModal,
  generateRecurringSelected: generateRecurringSelected,
  restoreTrashItem: restoreTrashItem,
  deleteTrashItemForever: deleteTrashItemForever,
  ...
  mergeSkipDedup: mergeSkipDedup,
  refreshMergePreview: refreshMergePreview,
  doMerge: doMerge,
  obRender: obRender,
  obCollect: obCollect,
  obNext: obNext,
